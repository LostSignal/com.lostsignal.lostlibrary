...
namespace Test
{
    ...
    public class BetterXRGrabInteractable : XRBaseInteractable
    {
        ...
        void PerformInstantaneousUpdate(XRInteractionUpdateOrder.UpdatePhase updatePhase)
        {
                ...
                updatePhase == XRInteractionUpdateOrder.UpdatePhase.OnBeforeRender)
            {
                if (trackPosition)
                {
                    ...
                    // {
                    //     var positionDelta = m_TargetWorldPosition - this.GetAttachPosition();  // BGISH
                    //     transform.position = m_TargetWorldPosition + positionDelta;
                    // }
                    // else
                    {
                        var positionDelta = m_TargetWorldPosition - this.GetAttachPosition();  // BGISH
...
#pragma warning restore IDE0029
            ...
            var attachOffset = this.GetAttachPosition() - attachPosition;
        ...
        protected override void OnSelectEntering(XRBaseInteractor interactor)
        {
            ...
            {
                m_TargetWorldPosition = this.GetAttachPosition();
                m_TargetWorldRotation = transform.rotation;
                m_CurrentAttachEaseTime = 0f;
            }
            else
            {
                m_TargetWorldPosition = this.GetAttachPosition();  // BGISH
            }

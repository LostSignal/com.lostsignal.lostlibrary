...
namespace Lost.Networking
{
    ...
    public class LiteNetLibServerTransport : IServerTransportLayer, INetEventListener
    {
        ...
        void IServerTransportLayer.SendData(long connectionId, byte[] data, uint offset, uint length, bool reliable)
        {
            if (this.peersHash.TryGetValue((int)connectionId, out NetPeer peer))
            {
                peer.Send(data, (int)offset, (int)length, reliable ? DeliveryMethod.ReliableOrdered : DeliveryMethod.Unreliable);
            }
        }
        ...
        void INetEventListener.OnNetworkReceive(NetPeer peer, NetPacketReader reader, DeliveryMethod deliveryMethod)
        {
            ...
            this.serverEvents.Enqueue(new ServerEvent
            {
                ...
                Data = data,
                Reliable = deliveryMethod != DeliveryMethod.Unreliable,
            });
        }

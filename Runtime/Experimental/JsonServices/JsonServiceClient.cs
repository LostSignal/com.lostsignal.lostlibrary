...
namespace Lost
{
    ...
    public abstract class JsonServiceClient
    {
        ...
        protected IEnumerator<U> SendPostRequest<T, U>(string url, T request)
        {
            ...
            UnityWebRequest unityWebRequest = UnityWebRequest.Post(this.Server + url, requestJson);
            unityWebRequest.SetRequestHeader("Content-Type", "application/json");
            //webRequest.SetRequestHeader("securityKey", this.GetSecurityKey(requestJson));
            ...
            if (string.IsNullOrEmpty(webRequest.webRequest.error) == false)
            {
                Debug.LogErrorFormat("UnityWebRequest Error: {0}", webRequest.webRequest.error);
                throw new System.Exception(webRequest.webRequest.error);
            }
            ...
            yield return JsonUtility.FromJson<U>(webRequest.webRequest.downloadHandler.text);
        }
